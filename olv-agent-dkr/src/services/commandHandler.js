...
const handlers = {
  ...
  // All system interfaces (physical + WG), each flagged with isWireGuard.
  // Physical interfaces are read-only on the client side; the flag tells
  // the UI which to render as editable vs informational.
  async listAllInterfaces() {
    const os = require('os');
    const wgNames = new Set(await wgService.listInterfaces());
    const all = os.networkInterfaces();
    const result = [];
    for (const [name, addrs] of Object.entries(all || {})) {
      // Skip pure loopback and docker bridges by convention; show WG even
      // if down (wgNames may include down interfaces that os.* omits).
      const ipv4 = addrs?.find(a => a.family === 'IPv4' && !a.internal);
      const ipv6 = addrs?.find(a => a.family === 'IPv6' && !a.internal);
      const isLoopback = addrs?.every(a => a.internal);
      if (isLoopback && !wgNames.has(name)) continue;
      result.push({
        name,
        address: ipv4 ? `${ipv4.address}${ipv4.cidr ? '/' + ipv4.cidr.split('/')[1] : ''}` : null,
        addressV6: ipv6 ? ipv6.address : null,
        mac: ipv4?.mac || addrs?.[0]?.mac || null,
        isWireGuard: wgNames.has(name),
        up: true, // os.networkInterfaces() only returns up interfaces
      });
    }
    // Ensure WG interfaces we know about but which may be down still appear.
    for (const name of wgNames) {
      if (result.some(i => i.name === name)) continue;
      result.push({ name, address: null, addressV6: null, mac: null, isWireGuard: true, up: false });
    }
    result.sort((a, b) => {
      if (a.isWireGuard !== b.isWireGuard) return a.isWireGuard ? -1 : 1;
  ...
  // Privileged variant — returns PSK as plain string. Used by migrate
  // only; never call from user-facing routes.
  async listPeersWithSecrets({ iface }) {
    return { peers: await wgService.listPeersWithSecrets(iface) };
  },
