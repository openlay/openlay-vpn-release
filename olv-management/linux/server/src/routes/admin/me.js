...
// POST /api/admin/me/encryption-key
// body: { public_key: <base64 X9.62 uncompressed P-256, 65 raw bytes> }
//
// Mirror of /signing-key but for the SE encryption key used to UNWRAP DEKs
// (data encryption keys) for high-value secrets like SSH private keys
// (see ssh_keys + ssh_key_dek_wraps). Idempotent — same key returns 200,
// different key replaces and updates the timestamp. JWT bearer is sufficient
// authority; we don't require a signed action because losing the encryption
// key is recoverable (re-import affected secrets) but locking out a freshly
// reinstalled admin is not.
router.post('/encryption-key', async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE users
          SET admin_encryption_public_key    = $1,
              admin_encryption_registered_at = NOW(),
              updated_at                     = NOW()
        WHERE id = $2`,
      [publicKey, req.user.id]
    );
...
// GET /api/admin/me/encryption-key — current registration status
router.get('/encryption-key', async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT admin_encryption_public_key, admin_encryption_registered_at
         FROM users WHERE id = $1`,
      [req.user.id]
    );
    if (rows.length === 0) return res.status(404).json({ error: 'User not found' });
    res.json({
      registered: !!rows[0].admin_encryption_public_key,
      public_key: rows[0].admin_encryption_public_key,
